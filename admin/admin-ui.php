...
class A3_Portfolio_Admin_UI
{
	 ...
	 */
	public $class_name = 'A3_Portfolio';
	...
	public function get_premium_video_data() {
		$premium_video_data = array(
				'box_title'    => __( 'Premium Version Enhanced Features', 'a3_portfolios' ),
				'image_url'    => WOO_DYNAMIC_GALLERY_IMAGES_URL. '/video.jpg',
				'video_url'    => 'https://www.youtube.com/embed/9dGw-ORfMIk?version=3&autoplay=1',
				'left_title'   => __( 'Premium Version Enhanced Features', 'a3_portfolios' ),
				'left_text'    => __( 'WooCommerce Dynamic Gallery Premium', 'a3_portfolios' )
									. "\n\n" . __( 'Quick Video showing the main (not all) enhanced features that are built into the WooCommerce Dynamic Gallery Premium version', 'a3_portfolios' ),
				'right_title'  => __( 'Developer Support and Premium Features', 'a3_portfolios' ),
				'right_text'   => __( 'Limited Time Offer. Purchase the Premium Version Lifetime License. That is a Lifetime of maintenance updates, feature upgrades and developer support for a once only fee. Offer ending soon.', 'a3_portfolios' )
	...
	public function upgrade_top_message( $echo = false, $setting_id = '' ) {
		$upgrade_top_message = sprintf( '<div class="pro_feature_top_message">'
			. __( 'Advanced Settings - Upgrade to the <a href="%s" target="_blank">%s License</a> to activate these settings.', 'a3_portfolios' )
			. '</div>'
			...
			, apply_filters( $this->plugin_name . '_' . $setting_id . '_pro_version_name', apply_filters( $this->plugin_name . '_pro_version_name', __( 'Pro Version', 'a3_portfolios' ) ) )
		);
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'a3_portfolios' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'a3_portfolios' ), $this->support_url );
			} else {
				...
				if ( FALSE !== stristr( $version_transient, '||' )
					...
					&& version_compare( get_option('a3_portfolio_version') , $version_info[0], '<' ) ) {
						...
						$version_message = sprintf( __( 'There is a new version <span class="a3rev-ui-new-plugin-version">%s</span> available, <a href="%s" target="_blank">update now</a> or download direct from <a href="%s" target="_blank">My Account</a> on a3rev.com', 'a3_portfolios' ),
							$version_info[0],
