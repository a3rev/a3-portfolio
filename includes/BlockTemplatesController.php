...
use A3Rev\Portfolio\BlockTemplateUtils;
...
class BlockTemplatesController
{
    ...
    protected function init()
    {
        ...
        //add_filter( 'theme_file_path', array( $this, '_template_part_file_path' ), 10, 2 );
    }

    public function _template_part_file_path($path, $file)
    {

        $fileString = isset($file) && is_string($file) ? str_replace('.html', '', $file) : '';

        $fileData = $fileString != '' ? explode('/', $fileString) : array();

        $slug = '';

        if (is_array($fileData) && count($fileData) > 0 && isset($fileData[1])) {
            $slug = $fileData[1];
        }

        if ($slug != '') {
            if (file_exists($this->template_parts_directory . '/' . $slug . '.html')) {
                $parent_theme_folders        = get_block_theme_folders(get_template());
                $child_theme_folders         = get_block_theme_folders(get_stylesheet());
                $child_theme_part_file_path  = get_stylesheet_directory(). '/' . $child_theme_folders['wp_template_part'] . '/content-portfolio.html';
                $parent_theme_part_file_path = get_template_directory(). '/' . $parent_theme_folders['wp_template_part'] . '/content-portfolio.html' ;
                ...
                if ((0 === $slug ) && file_exists($template_part_file_path)) {
                    return $path;
                } else {
                    $path = $this->template_parts_directory . '/' . $slug . '.html';
                }
            }
        }
     ...
     * database or a template file in `a3-portfolio/templates/block-templates/`)
    ...
    public function maybe_return_blocks_template($template, $id, $template_type)
    {
        ...
        // Theme-based template didn't exist, try switching the theme to a3-portfolio and try again. This function has
        // been unhooked so won't run again.
        add_filter('get_block_file_template', array( $this, 'get_single_block_template' ), 10, 3);
        $maybe_template = function_exists('gutenberg_get_block_template') ?
            gutenberg_get_block_template('a3-portfolio//' . $slug, $template_type) :
            get_block_template('a3-portfolio//' . $slug, $template_type);
    ...
    public function remove_theme_templates_with_custom_alternative($templates)
    {
        ...
        // for `a3-portfolio` in $template->source here because a3-portfolio templates won't have been added to $templates
    ...
    public function get_block_templates_from_db($slugs = array(), $template_type = 'wp_template')
    {
        $check_query_args = array(
            ...
            'tax_query'      => array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query
                array(
                    'taxonomy' => 'wp_theme',
                    'field'    => 'name',
                    'terms'    => array( 'a3-portfolio', get_stylesheet(), 'portfolio' ),
                ),
            ),
        );
        ...
        $saved_portfolio_templates = $check_query->posts;

        return array_map(
            function ($saved_portfolio_template) {
                return BlockTemplateUtils::gutenberg_build_template_result_from_post($saved_portfolio_template);
            },
            $saved_portfolio_templates
        );
    }
     ...
     */
    public function get_block_templates_from_a3_portfolio($slugs, $already_found_templates, $template_type = 'wp_template')
    {
        ...
        foreach ($template_files as $template_file) {
            ...
            // If the theme has an archive-portfolio.html template, but not a taxonomy-portfolio_cat.html template let's use the themes archive-portfolio.html template.
            if (BlockTemplateUtils::template_is_eligible_for_portfolio_archive_fallback($template_slug)) {
                $template_file = BlockTemplateUtils::get_theme_template_path('archive-portfolio');
    ...
    public function get_block_templates($slugs = array(), $template_type = 'wp_template')
    {
        ...
        $templates_from_portfolio = $this->get_block_templates_from_a3_portfolio($slugs, $templates_from_db, $template_type);
        return array_merge($templates_from_db, $templates_from_portfolio);
    }
    ...
    public function render_block_template()
    {
        ...
        if (( is_post_type_archive('portfolio') || is_post_type_archive('portfolios') ) &&
            ! BlockTemplateUtils::theme_has_template('archive-portfolio') &&
            $this->block_template_is_available('archive-portfolio')
        ) {
            add_filter('portfolio_has_block_template', '__return_true', 10, 0);
        } elseif (is_singular('portfolio') &&
            ! BlockTemplateUtils::theme_has_template('single-a3-portfolio') &&
            $this->block_template_is_available('single-a3-portfolio')
        ) {
            add_filter('portfolio_has_block_template', '__return_true', 10, 0);
        } elseif (( is_tax('portfolio_cat') ) &&
            ! BlockTemplateUtils::theme_has_template('taxonomy-portfolio_cat') &&
            $this->block_template_is_available('taxonomy-portfolio_cat')
        ) {
            add_filter('portfolio_has_block_template', '__return_true', 10, 0);
        } elseif (( is_tax('portfolio_tag') ) &&
            ! BlockTemplateUtils::theme_has_template('taxonomy-portfolio_tag') &&
            $this->block_template_is_available('taxonomy-portfolio_tag')
        ) {
            add_filter('portfolio_has_block_template', '__return_true', 10, 0);
        }
    }
