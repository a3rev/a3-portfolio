...
-----------------------------------------------------------------------------------*/
class A3_Portfolio_Uploader extends A3_Portfolio_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $attachment_id = 0, $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $strip_methods = true ) {
		...
		$output .= '<input id="upload_'.$id_attribute.'" class="a3rev-ui-upload-button a3_upload_button button" type="button" value="'.__( 'Upload', 'a3_portfolios' ).'" /> '.$description;
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'a3_portfolios' );
...
global $a3_portfolio_uploader;
$a3_portfolio_uploader = new A3_Portfolio_Uploader();
