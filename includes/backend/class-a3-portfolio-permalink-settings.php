...
class A3_Portfolio_Permalink_Settings {
	...
	public function settings_init() {
		...
		add_settings_section( 'a3-portfolio-permalink', __( 'a3 Portfolio Permalinks', 'a3_portfolios' ), array( $this, 'settings' ), 'permalink' );
	...
	public function portfolio_category_slug_input() {
		?>
		<input name="a3_portfolio_category_slug" type="text" class="regular-text code" value="<?php echo esc_attr( $this->permalinks['category_base'] ); ?>" placeholder="<?php echo esc_attr_x('portfolio-category', 'slug', 'a3_portfolios') ?>" />
		<?php
	}

	public function portfolio_tag_slug_input() {
		?>
		<input name="a3_portfolio_tag_slug" type="text" class="regular-text code" value="<?php echo esc_attr( $this->permalinks['tag_base'] ); ?>" placeholder="<?php echo esc_attr_x('portfolio-tag', 'slug', 'a3_portfolios') ?>" />
		<?php
	}
		...
		<table class="form-table a3_portfolio_permalink_structure_table">
			<tbody>
				...
				<?php if ( $portfolio_page_id ) : ?>
					<tr>
						<th><label><input name="portfolio_permalink" type="radio" value="<?php echo $structures[1]; ?>" class="a3_portfolio_permalink" <?php checked( $structures[1], $portfolio_permalink ); ?> /> <?php _e( 'Main page base', 'a3_portfolios' ); ?></label></th>
						<td><code><?php echo esc_html( home_url() ); ?>/<?php echo esc_html( $base_slug ); ?>/sample-portfolio/</code></td>
					</tr>
					<tr>
						<th><label><input name="portfolio_permalink" type="radio" value="<?php echo $structures[2]; ?>" class="a3_portfolio_permalink" <?php checked( $structures[2], $portfolio_permalink ); ?> /> <?php _e( 'Main page base with category', 'a3_portfolios' ); ?></label></th>
