...
<div data-attribute-id="<?php echo esc_attr( $attribute_id ); ?>" data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>" class="portfolio_attribute a3-metabox-item closed <?php echo esc_attr( implode( ' ', $metabox_class ) ); ?>" rel="<?php echo $position; ?>">
	<h3>
		<a href="#" class="remove_row delete"><?php _e( 'Remove', 'a3_portfolios' ); ?></a>
			...
			<tbody>
				<tr>
					<td class="attribute_name">
						<label><?php _e( 'Name', 'a3_portfolios' ); ?>:</label>
					...
					<td rowspan="2">
						...
						<?php if ( 'select' === $attribute_taxonomy->attribute_type ) : ?>
							<label><?php _e( 'Value(s)', 'a3_portfolios' ); ?>:</label>

							<select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'a3_portfolios' ); ?>" class="multiselect attribute_values a3-portfolio-enhanced-select a3rev-ui-multiselect chzn-select" name="attribute_values[<?php echo $attribute_id; ?>][]" style="width: 100%;">
								<?php
							...
							<button class="button plus select_all_attributes"><?php _e( 'Select all', 'a3_portfolios' ); ?></button>
							<button class="button minus select_no_attributes"><?php _e( 'Select none', 'a3_portfolios' ); ?></button>
							<button class="button fr plus add_new_attribute"><?php _e( 'Add new', 'a3_portfolios	' ); ?></button>

						<?php else : ?>
							<label><?php _e( 'Value', 'a3_portfolios' ); ?>:</label>

							<input type="text" name="attribute_values[<?php echo $attribute_id; ?>]" value="<?php
								echo esc_attr( $attribute['value'] );
							?>" placeholder="<?php echo esc_attr( __( 'Enter the value here', 'a3_portfolios' ) ); ?>" />
				...
				<tr>
					<td>
						<div><label><input type="checkbox" class="checkbox" <?php checked( $attribute['is_visible_expander'], 1 ); ?> name="attribute_visibility[<?php echo $attribute_id; ?>][expander]" value="1" /> <?php _e( 'Visible on Item expander', 'a3_portfolios' ); ?></label></div>
						<div><label><input type="checkbox" class="checkbox" <?php checked( $attribute['is_visible_post'], 1 ); ?> name="attribute_visibility[<?php echo $attribute_id; ?>][post]" value="1" /> <?php _e( 'Visible on Item Post', 'a3_portfolios' ); ?></label></div>
					</td>
				</tr>
