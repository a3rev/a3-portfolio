	...
	public function admin_forms( $options, $form_key, $option_name = '', $form_messages = array() ) {
			...
			switch( $value['type'] ) {
				...
				case 'google_api_key':
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
							...
							<label for="<?php echo esc_attr( $this->google_api_key_option ); ?>"><?php esc_html_e( 'Google Fonts API', 'a3-portfolio' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							<input
								...
								/> <span class="description" style="margin-left:5px;"><?php esc_html_e( 'ON to connect to Google Fonts API and have auto font updates direct from Google.', 'a3-portfolio' ); ?></span>
				...
				case 'google_map_api_key':
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo wp_kses_post( $tip ); ?>
							<label for="<?php echo esc_attr( $this->google_map_api_key_option ); ?>"><?php esc_html_e( 'Google Maps API', 'a3-portfolio' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							<input
								...
								/> <span class="description" style="margin-left:5px;"><?php esc_html_e( 'Switch ON to connect to Google Maps API', 'a3-portfolio' ); ?></span>
				...
				case 'manual_check_version':
					...
					if ( is_multisite() ) {
						$version_message = __( 'Sorry, this feature just for Network Admin.', 'a3-portfolio' );
					} else {
						global $a3_dashboard_plugin_requirement;

						if ( ! $a3_dashboard_plugin_requirement->is_installed() ) {
							$version_message = sprintf( __( 'You need to install and activate the <a title="" href="%s" target="_parent">a3rev Dashboard plugin</a> for manage version and get auto upgrade to latest version.', 'a3-portfolio' ), $a3_dashboard_plugin_requirement->install_url() );
						} elseif ( ! $a3_dashboard_plugin_requirement->is_activated() ) {
							$version_message = sprintf( __( 'You need to activate the <a title="" href="%s" target="_parent">a3rev Dashboard plugin</a> for manage version and get auto upgrade to latest version.', 'a3-portfolio' ), $a3_dashboard_plugin_requirement->activate_url() );
						} elseif ( function_exists( 'is_a3_club_membership' ) && ! is_a3_club_membership() ) {
							$version_message = sprintf( __( 'You need to go to <a title="" href="%s">a3 Dashboard Main page</a> for login before check for Update. Use your account creds on <a href="https://a3rev.com" target="_parent">a3rev.com</a> to login.', 'a3-portfolio' ), self_admin_url( 'admin.php?page=a3rev-dashboard' ) );
						} else {
							$check_version_class = 'a3rev-ui-latest-version-message';
							$version_message = sprintf( __( 'Go to <a href="%s">a3 Dashboard Main page</a> and hit on <strong>CHECK NOW</strong> button to manual check for Update.', 'a3-portfolio' ), self_admin_url( 'admin.php?page=a3rev-dashboard' ) );
						}
					}
				...
				case 'onoff_toggle_box' :
					...
					?><tr valign="top">
						<th scope="row" class="titledesc">
                        	<?php echo wp_kses_post( $tip ); ?>
							<label for="<?php echo esc_attr( $this->toggle_box_open_option ); ?>"><?php esc_html_e( 'Open Box Display', 'a3-portfolio' ); ?></label>
						</th>
						<td class="forminp forminp-onoff_checkbox forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							<input
								...
								<?php $this->esc_attribute_array_e( $custom_attributes ); // WPCS: XSS ok. ?>
								/> <span class="description" style="margin-left:5px;"><?php esc_html_e( 'ON and each admin panel setting box OPEN | CLOSED position are saved each time changes are SAVED.', 'a3-portfolio' ); ?></span>
                        </td>
					</tr><?php
				break;
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							<input
									...
									checked_label="<?php esc_attr_e( 'ON', 'a3-portfolio' ); ?>"
									unchecked_label="<?php esc_attr_e( 'OFF', 'a3-portfolio' ); ?>"
									type="checkbox"
									value="1"
				...
				case 'image_size' :
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
							...
							<label><?php esc_html_e( 'Width', 'a3-portfolio' ); ?> <input name="<?php $this->esc_attribute_name_e( $name_attribute ); ?>[width]" id="<?php echo esc_attr( $id_attribute ); ?>-width" type="text" class="a3rev-ui-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>-width" value="<?php echo esc_attr( $width ); ?>" /></label>
	
							<label><?php esc_html_e( 'Height', 'a3-portfolio' ); ?> <input name="<?php $this->esc_attribute_name_e( $name_attribute ); ?>[height]" id="<?php echo esc_attr( $id_attribute ); ?>-height" type="text" class="a3rev-ui-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>-height" value="<?php echo esc_attr( $height ); ?>" /></label>
	
							<label><?php esc_html_e( 'Hard Crop', 'a3-portfolio' ); ?> <input name="<?php $this->esc_attribute_name_e( $name_attribute ); ?>[crop]" id="<?php echo esc_attr( $id_attribute ); ?>-crop" type="checkbox" class="a3rev-ui-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>-crop" <?php echo checked( 1, $option_value['crop'], false ); ?> /></label>
						...
						<td class="forminp">
                           ...
                           <!-- Font Face -->
							<select
								...
								>
								<optgroup label="<?php esc_attr_e( '-- Default Fonts --', 'a3-portfolio' ); ?>">
                                <?php
                                ...
                                <optgroup label="<?php esc_attr_e( '-- Google Fonts --', 'a3-portfolio' ); ?>">
                                <?php
                           ...
                           <!-- Preview Button -->
                           <div class="a3rev-ui-typography-preview"><a href="#" class="a3rev-ui-typography-preview-button button submit-button" title="<?php esc_attr_e( 'Preview your customized typography settings', 'a3-portfolio'); ?>"><span>&nbsp;</span></a></div>
				...
				case 'border':
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-border_corner">
                           ...
                           <!-- Preview Button -->
                           <div class="a3rev-ui-settings-preview"><a href="#" class="a3rev-ui-border-preview-button a3rev-ui-settings-preview-button button submit-button" title="<?php esc_attr_e( 'Preview your customized border settings', 'a3-portfolio' ); ?>"><span>&nbsp;</span></a></div>
                           <span class="description" style="margin-left:5px;"><?php esc_html_e( '0px = No Border', 'a3-portfolio' ); ?></span>
                           ...
                           <!-- Border Corner : Rounded or Square -->
								<input
                                    ...
                                    checked_label="<?php esc_attr_e( 'Rounded', 'a3-portfolio' ); ?>"
                                    unchecked_label="<?php esc_attr_e( 'Square', 'a3-portfolio' ); ?>"
                                    type="checkbox"
                                    value="rounded"
							...
							<!-- Border Rounded Value -->
								<div class="a3rev-ui-border-corner-value-container">
                                	<div class="a3rev-ui-border_corner-top_left">
                                        <span class="a3rev-ui-border_corner-span"><?php esc_html_e( 'Top Left Corner', 'a3-portfolio' ); ?></span>
                                    ...
                                    <div class="a3rev-ui-border_corner-top_right">
                                        <span class="a3rev-ui-border_corner-span"><?php esc_html_e( 'Top Right Corner', 'a3-portfolio' ); ?></span> 
                                        <div class="a3rev-ui-slide-container">
                                            <div class="a3rev-ui-slide-container-start">
                                                <div class="a3rev-ui-slide-container-end">
                                    ...
                                    <div class="a3rev-ui-border_corner-bottom_right">
                                        <span class="a3rev-ui-border_corner-span"><?php esc_html_e( 'Bottom Right Corner', 'a3-portfolio' ); ?></span> 
                                        <div class="a3rev-ui-slide-container">
                                            <div class="a3rev-ui-slide-container-start">
                                                <div class="a3rev-ui-slide-container-end">
                                    ...
                                    <div class="a3rev-ui-border_corner-bottom_left">
                                        <span class="a3rev-ui-border_corner-span"><?php esc_html_e( 'Bottom Left Corner', 'a3-portfolio' ); ?></span>
                                        <div class="a3rev-ui-slide-container"> 
                                            <div class="a3rev-ui-slide-container-start">
                                                <div class="a3rev-ui-slide-container-end">
				...
				case 'border_styles':
					...
					?><tr valign="top">
						...
						<td class="forminp">
                           ...
                           <!-- Preview Button -->
                           <div class="a3rev-ui-settings-preview"><a href="#" class="a3rev-ui-border-preview-button a3rev-ui-settings-preview-button button submit-button" title="<?php esc_attr_e( 'Preview your customized border styles settings', 'a3-portfolio' ); ?>"><span>&nbsp;</span></a></div>
                           <span class="description" style="margin-left:5px;"><?php esc_html_e( '0px = No Border', 'a3-portfolio' ); ?></span>
                           </div>
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-box_shadow">
							...
							<select
                                ...
                                data-placeholder="<?php esc_attr_e( 'Horizontal Shadow', 'a3-portfolio' ); ?>"
								>
								<?php
									for ( $i = -20; $i <= 20; $i++ ) {
										?>
							...
							<select
                                ...
                                data-placeholder="<?php esc_attr_e( 'Vertical Shadow', 'a3-portfolio' ); ?>"
								>
								<?php
									for ( $i = -20; $i <= 20; $i++ ) {
										?>
                           ...
                           <!-- Box Blur Distance -->
							<select
                                ...
                                data-placeholder="<?php esc_attr_e( 'Blur Distance', 'a3-portfolio' ); ?>"
								>
								<?php
									for ( $i = 0; $i <= 20; $i++ ) {
										?>
                           ...
                           <!-- Box Spread -->
							<select
                                ...
                                data-placeholder="<?php esc_attr_e( 'Spread Size', 'a3-portfolio' ); ?>"
								>
								<?php
									for ( $i = 0; $i <= 20; $i++ ) {
										?>
                           ...
                           <!-- Box Shadow Inset -->
                                <input
                                    ...
                                    checked_label="<?php esc_attr_e( 'INNER', 'a3-portfolio' ); ?>"
                                    unchecked_label="<?php esc_attr_e( 'OUTER', 'a3-portfolio' ); ?>"
                                    type="checkbox"
                                    value="inset"
                           ...
                           <input
                            ...
                            <!-- Preview Button -->
                           <div class="a3rev-ui-settings-preview"><a href="#" class="a3rev-ui-box_shadow-preview-button a3rev-ui-settings-preview-button button submit-button" title="<?php esc_attr_e( 'Preview your customized box shadow settings', 'a3-portfolio'); ?>"><span>&nbsp;</span></a></div>   
                           </div>
                           <div style="clear:both;"></div>
                           </div>
						</td>
		...
		<?php do_action( $this->plugin_name . '-' . trim( $form_key ) . '_settings_end' ); ?>
            <p class="submit">
                    ...
                    <input type="submit" value="<?php esc_attr_e('Save changes', 'a3-portfolio'); ?>" class="button button-primary" name="bt_save_settings" />
                    <input type="submit" name="bt_reset_settings" class="button" value="<?php esc_attr_e('Reset Settings', 'a3-portfolio'); ?>"  />
